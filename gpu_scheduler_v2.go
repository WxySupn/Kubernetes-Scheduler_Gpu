...
func GetMaxGpuNode(GpuNeed int64, clientset *kuber.Clientset) (int64,string){
	var(
		i int
		z int
		GpuNum int64
	...
	for i = 0; i < len(nodes.Items); i++{
		...
		if (GpuNum > GpuNeed) && (GpuNum - GpuNeed < GpuNum - MaxGpuNum){
			MaxGpuNum = GpuNum
...
func scheduler_agroup(label_work string, clientset *kuber.Clientset){
	...
	for GpuNums == 0{
		GpuNums, NodeName = GetMaxGpuNode(GpuNeed, clientset)
